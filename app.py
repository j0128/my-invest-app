...
def fetch_data(tickers):
    ...
    # 顯示進度條 (因為一支一支抓會稍慢)
    progress_bar = st.progress(0, text="正在建立加密連線...")
    ...
    for i, t in enumerate(all_tickers):
        try:
            ...
            # 使用最原始的 Ticker.history 方法，這最穩定
            # auto_adjust=True 會自動處理分割配息，回傳的 Close 就是調整後價格
...
def plot_kline_chart(ticker, df_close, df_open, df_high, df_low):
    ...
    try:
        lookback = 120
        # 這裡不需要複雜的 try-except，因為數據結構已經是乾淨的 DataFrame
        dates = df_close.index[-lookback:]
        
        # 處理如果某些股票數據較短的情況
        if len(df_close[ticker].dropna()) < 1: return None
        
        fig = go.Figure()
        fig.add_trace(go.Candlestick(
            x=dates, 
            open=df_open[ticker].iloc[-lookback:], 
...
NVDA, 10000"""
    ...
    # 這裡的 Spinner 訊息會被內部的 Progress Bar 覆蓋，但為了美觀保留
    with st.spinner("Alpha 正在連接交易所 (模式: 序列下載)..."):
        df_close, df_open, df_high, df_low = fetch_data(tickers_list)
            
    if df_close.empty:
        st.error("所有數據下載失敗，請檢查網路或代號。")
        return
    ...
    with t_col1:
        if table_data: st.dataframe(pd.DataFrame(table_data), use_container_width=True, hide_index=True)
    with t_col2:
        if total_value > 0:
